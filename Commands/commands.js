const command = [
  '!', 'a!kiss @member', 'a!shitpost', 'a!fuck @member', 'a!kill @member',
  'a!hug @member', 'a!masturbation @member', 'a!slap @member',
  'a!curse @member', 'a!vidtogif', 'a!question', 'a!leaderboard',
  'a!demotivator', 'a!avatar', 'a!balance'
];
...
function initAutocommand() {
  let autos = document.querySelectorAll('.auto-firstname');

  autos.forEach((auto) => {
      auto.innerHTML = command[rand(0, command.length)];
  });
}

function initAutoAbout() {
  let autos = document.querySelectorAll('.auto-lastname');

  autos.forEach((auto) => {
      auto.innerHTML = About[rand(0, About.length)];
  });
}
